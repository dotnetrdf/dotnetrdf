namespace VDS.RDF.Dynamic
{
    ...
    public class DynamicGraphDynamicGraph_StringDictionary
    {
        [Fact]
        ...
        public void Get_index_returns_dynamic_subject()
        {
            ...
            var expected = d.CreateUriNode(new Uri(s));
            var actual = d[s];
...
");
        ...
        public void CopyTo_creates_pairs_with_dynamic_value()
        {
            ...
            Assert.Equal(key, s);
            Assert.Equal(value, g.CreateUriNode(new Uri(s)));
...
");
        ...
        public void TryGetValue_returns_dynamic_subject()
        {
            ...
            Assert.True(condition);
            Assert.Equal(value, d.CreateUriNode(new Uri(s)));
            Assert.IsType<DynamicNode>(value);
        }
    }
}
