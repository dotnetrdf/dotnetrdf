...
namespace VDS.RDF.Interop.SemWeb
{
        ...
        : StatementSource, QueryableSource
    {
        ...
        private List<ITriplePattern> TemplateToTriplePatterns(Statement template)
        {
            ...
            if (template.Subject is BNode && !(template.Subject is Variable)) patterns.Add(new FilterPattern(new UnaryExpressionFilter(new IsBlankFunction(new VariableTerm(s.VariableName)))));
            if (template.Predicate is BNode && !(template.Predicate is Variable)) patterns.Add(new FilterPattern(new UnaryExpressionFilter(new IsBlankFunction(new VariableTerm(p.VariableName)))));
            if (template.Object is BNode && !(template.Object is Variable)) patterns.Add(new FilterPattern(new UnaryExpressionFilter(new IsBlankFunction(new VariableTerm(o.VariableName)))));
