    ...
    class SparqlPathParser
    {
        ...
        private ISparqlPath TryParsePathPrimary(SparqlQueryParserContext context, Queue<IToken> tokens)
        {
            ...
            switch (next.TokenType)
            {
                case Token.URI:
                case Token.QNAME:
                    path =  new Property(new UriNode(UriFactory.Create(Tools.ResolveUriOrQName(next, context.Query.NamespaceMap, context.Query.BaseUri))));
                    break;

                case Token.KEYWORDA:
                    path =  new Property(new UriNode(UriFactory.Create(RdfSpecsHelper.RdfType)));
                    break;
        ...
        private Property TryParsePathOneInPropertySet(SparqlQueryParserContext context, Queue<IToken> tokens, out bool inverse)
        {
            ...
            switch (next.TokenType)
            {
                case Token.URI:
                case Token.QNAME:
                    return new Property(new UriNode(UriFactory.Create(Tools.ResolveUriOrQName(next, context.Query.NamespaceMap, context.Query.BaseUri))));

                case Token.KEYWORDA:
                    return new Property(new UriNode(UriFactory.Create(RdfSpecsHelper.RdfType)));
